...
@Validated
@Slf4j
public class AuthenticationControllerForPhone {
	...
	@Autowired
	public AuthenticationControllerForPhone(AuthenticationService service, LimiterService limiterService) {
	...
	public ResponseEntity<AuthenticationResponse> register(
			@Valid @RequestBody RegisterRequestByPhone request
	) {
		AuthenticationResponse response = service.registerByPhone(request);
	...
	public ResponseEntity<AuthenticationResponse> authenticate(
			@Valid @RequestBody AuthenticationRequestByPhone request
	) {
		AuthenticationResponse response = service.authenticateByPhone(request);
		HttpHeaders headers = setHttpHeaders(response);
		return ResponseEntity.ok().headers(headers).build();
	}


	@PostMapping("/sign-up/by-phone/request-code")
	@ResponseStatus(HttpStatus.OK)
	public void requestSMS(@Valid @RequestBody SMSCodeRequest request) {
		if (bucket.tryConsume(request.getPhone())) {
			service.requestCodeSMS(request);
		} else {
